...
export default function ComboWizard({ currentStep, nodes, onAddNode, onUndo, onClear }) {
  ...
  useEffect(() => {
    if (isOpponentTurn && isAiMode && recommendedMoves.length > 0 && !isMistake) {
      ...
      const timer = setTimeout(() => {
        // --- 🧠 TACTICAL AI WEIGHTING ALGORITHM ---
        let bestMove = recommendedMoves[0];
        let maxScore = -999;
        const userTags = lastNode?.tags || [];

        recommendedMoves.forEach((move) => {
          let score = Math.random() * 2; // base variance so it's not identical every time
  ...
  }, [isOpponentTurn, isAiMode, recommendedMoves, onAddNode, nodes.length, currentPhase, isMistake]);
  ...
  const handleAdd = useCallback(() => {
    ...
    playClash();
    setSelectedMove('');
  }, [selectedMove, availableMoves, nodes.length, onAddNode]);
          ...
          {isComplete ? (
            ...
            <>
              ...
              <select
                ...
                className="w-full bg-[var(--color-parchment-light)] shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] px-4 py-4 md:py-3 border-[2px] border-[var(--color-ink-black)] rounded-none text-sm font-body font-bold text-[var(--color-ink-black)] focus:outline-none focus:border-[var(--color-ink-red)] transition-all"
              >
                <option value="" disabled className="bg-[var(--color-parchment)] italic">
                  {t('wizard_select')}
                </option>
                {/* ... Options handled the same ... */}
                {recommendedMoves.length > 0 && (
                  <optgroup label={t('wizard_recommended')} className="bg-[var(--color-parchment-dark)] text-[var(--color-ink-red)] font-display">
                    {recommendedMoves.map((move) => (
                      <option key={move.id} value={move.id} className="text-[var(--color-ink-black)] font-body font-normal bg-[var(--color-parchment-light)]">
                        {t(move.nameKey)} — {move.master} [{move.tags?.join(', ')}]
                      </option>
                    ))}
                  </optgroup>
                )}
                <optgroup label={t('wizard_other')} className="bg-[var(--color-parchment-dark)] text-[var(--color-ink-faded)] font-display">
                  {otherMoves.map((move) => (
                    <option key={move.id} value={move.id} className="text-[var(--color-ink-black)] font-body font-normal bg-[var(--color-parchment-light)]">
                      {t(move.nameKey)} — {move.master} [{move.tags?.join(', ')}]
                    </option>
